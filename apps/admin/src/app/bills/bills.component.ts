          ...
          el.addEventListener('click', (e) => {
            ...
            try {
              ...
              const lines = [
                ...
                ``,
                `*Items:*`,
                ...data.items.map((i: any) => `${i.name} x${i.qty} — ₦${(i.price / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}`),
                ``,
                `Subtotal: ₦${(data.subtotal / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}`,
                `Service Charge: ₦${(data.serviceCharge / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}`,
                ...(data.discount ? [`Discount: -₦${(data.discount / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}`] : []),
                `*Total: ₦${(data.total / 100).toLocaleString('en-NG', { minimumFractionDigits: 2 })}*`,
                ``,
