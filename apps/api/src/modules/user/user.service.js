...
let UserService = class UserService {
    ...
    async createWaiter(dto, businessId) {
        if (!businessId) {
        ...
        try {
            ...
            while (!pinIsUnique && attempts < 10) {
                ...
                const pinTaken = await Promise.all(existing.map((w) => (w.pin_hash ? bcrypt.compare(pin, w.pin_hash) : Promise.resolve(false))));
    ...
    async resetWaiterPin(waiterId, businessId) {
    ...
    async removeWaiter(id, businessId) {
        const waiter = await this.userRepository.findOne({
            where: { id, business_id: businessId, role: UserRole.WAITER },
        });
        if (!waiter)
            throw new NotFoundException('Waiter not found or does not belong to your business');
...
UserService = __decorate([
    ...
    __param(1, InjectRepository(Branch)),
